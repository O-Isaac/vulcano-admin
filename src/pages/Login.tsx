...
export default function Login() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Intermediate animation duration
            setTimeout(() => {
                navigate('/');
            }, 2000);
        } catch (err) {
    ...
    return (
        <div className="relative flex flex-col min-h-screen bg-white font-sans text-gray-900 overflow-hidden">
            {/* Intermediate Transition Page */}
            {transitioning && (
                <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center animate-portal-expand">
                    <div className="absolute inset-0 opacity-40">
                        <InteractiveBlob />
                    </div>
                    <div className="relative z-10 flex flex-col items-center gap-6">
                        <div className="p-5 bg-white/10 backdrop-blur-md rounded-3xl border border-white/20 animate-pulse">
                            <Hammer className="w-16 h-16 text-white" fill="white" strokeWidth={0} />
                        </div>
                        <h1 className="text-7xl font-bold text-white tracking-[0.2em] uppercase animate-text-shine">
                            Vulcano
                        </h1>
                        <div className="w-32 h-1 bg-white/20 rounded-full overflow-hidden">
                            <div className="h-full bg-white animate-[progress_2s_ease-in-out_forwards]" style={{ width: '0%' }} />
                        </div>
                    </div>
                </div>
            )}

            <div className={`flex flex-col flex-1 transition-all duration-700 ${transitioning ? 'scale-110 blur-2xl opacity-0' : ''}`}>
                <Header />
